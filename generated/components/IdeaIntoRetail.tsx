...
const IdeaIntoRetail: React.FC<Props> = ({
  ink,
  accent,
  backgroundBase,
  backgroundSrc,
  ...
  const { cy, k } = React.useMemo(() => {
    ...
    for (let f = 1; f <= frame; f++) {
      ...
      vk += (tk - kk) * CAM_STIFF - vk * CAM_DAMP;
      kk += vk;
      vcy += (tcy - ccy) * CAM_STIFF - vcy * CAM_DAMP;
      ccy += vcy;
    }
    return { cy: ccy, k: kk };
  ...
  const bgY = -(cy - CAM_CY[0]) * k * parallax - frame * 0.24;
  const bgScale = 1 + (k - 1) * 0.16;
              ...
              return (
                <g key={`p-${idx}`}>
                  ...
                  <rect
                    ...
                    fill={
                      refine > 0
                        ? interpolateColors(clamp01((frame - step.at) / 5), [0, 1], [ink, accent])
                        : ink
                    }
                    opacity={
