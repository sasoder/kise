...
// Tighter-framed variant: the agent/target row sits lower and HF higher, so
// the whole story hugs the vertical centre.
const AGENT = {x: 300, y: 540};
const TARGET = {x: 845, y: 515};
const HF = {x: 540, y: 1220};
const HF_W = 330;
...
const TRAIL = [
  {p0: [385, 562], c1: [545, 485], c2: [690, 508], p3: [706, 646]},
  {p0: [706, 646], c1: [726, 785], c2: [668, 918], p3: [562, 1085]},
] as const;
...
export type WarningShotHackTightProps = z.infer<typeof schema>;

export const defaultProps: WarningShotHackTightProps = schema.parse({
  ink: '#FFFFFF',
  accent: '#48D9FF',
...
const WarningShotHackTight: React.FC<WarningShotHackTightProps> = ({
  ink,
  accent,
  shadow,
  background,
  ...
  const camY =
    620 +
    mv(beats.hfEnter - 1, beats.hfEnter + 29, 260) +
    mv(beats.trailStart + 2, beats.impact, 200) +
    mv(beats.impact + 14, beats.prePing + 1, -120) +
    mv(beats.warningShot, beats.warningShot + 28, 30) +
    mv(beats.echo + 5, beats.echo + 45, -30) +
...
export default WarningShotHackTight;
