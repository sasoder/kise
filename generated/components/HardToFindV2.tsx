...
// The clip's shared module. V2 takes the second half of it: the tile still
// carries the Equifax wordmark knocked out of the house material, but a breach
// is now the tile BREAKING OPEN (`CrackedBrandTile`) and the data that comes out
// of the break is a stack of personal records (`RecordCard`), not a stream of
// abstract dots. Imported, never restated; the V2 exports were added by this
// builder and nothing above them was touched.
import {
  CrackedBrandTile,
  EFX_REST,
  RecordCard,
...
//   f30 moment  — the tag lands on the line INTACT; the price starts to fall;
//                 the tile starts to break open
//   f34         — the break is wide enough to lose records through: the first
//                 of them falls out of it
//   f38         — the tile is fully open, and the camera lets go (the held
//                 breath f30-38 ends on the same frame)
...
//   f92 end     — speech ends; the last records are still falling into the heap
//   f102        — the twenty-second record settles; the tail holds to f108
//
// ---------------------------------------------------------------------------
// V2 (2026-09-15). The user, on V1: "the balls dropping from the company logo
// is way too abstract and doesn't feel motivated. Replace the dots with
// documents so it shows actual data." V1 IS OTHERWISE APPROVED AND UNTOUCHED —
// the twenty-year mapping, the head's arc metric, the camera track, the tag's
// descent and its landing, the drop, the pull-back and the final frame are all
// carried over exactly, and this file is byte-identical to HardToFind.tsx for
// every frame up to f30. ONLY THE MARKER'S LEAK CHANGES, and it changes into
// the thing the leak was always standing for:
//
//   * THE TAG BREAKS. It lands intact on the top of the cliff at f30 and then
//     splits down a jagged crack, the two halves prised apart at the foot,
//     f30 -> f38. The kraft shows through the break. No ink click on it: the
//     amber leg the price is laying down the same cliff on the same beat is
//     this cut's one accent.
//   * WHAT FALLS OUT IS RECORDS. 17 x 22 px personal files — a person and two
//     lines knocked out of the house tile — one every two frames from f34,
//     tumbling out of the break, down the right of the cliff, into a heap at
//     the foot of the notch: twenty-two of them in rows of 6, 5, 4, 3, 2, 2,
//     standing on the low itself. (REV 1 of V2: the cards were 12 x 16 at a rate
//     of 3 and sixteen of them, and at the moment itself that read as a trickle
//     of chips rather than as files. See REC.)
//   * THE HEAP IS WHAT MARKS THE MOMENT IN THE WIDE SHOT. At k 1.00 the amber
//     notch has a small pale tag above it and a small pale pile under it, on a
//     line that has gone on to triple. That is the picture the line is about.
//
// ---------------------------------------------------------------------------
...
//
//   PHASE 2 · THE DROP AND THE BREAK (f30 -> f42, "moment")
...
//     stays where it landed, on the top of the cliff, and over those same eight
//     frames it BREAKS OPEN. From f34 records start falling out of the break
//     and down the right of the cliff. The head then rests on the low f42-f46,
//     with the spill and the pull-back carrying the hold.
//
...
//     pulls back k 2.28 -> 1.00 on one move, landed by "chart". The broken tag
//     shrinks with the world into a small split tag at the notch, records still
//     dropping out of it, and the amber leg becomes one notch among several in
//     twenty years of chart.
//
//   TAIL (f92 -> f108)
//     k 1.00 on `sway`. The last records are still falling — the twenty-second
//     settles at f102 — so the only motion in the tail is at the notch, and the
//     eye is led back to the thing it can barely find.
//
...
//
// V2's OWN DEVIATIONS, all argued where they are set:
//   * THE CRACK'S MOUTH IS AT THE FOOT, and the halves lean back together at
//     the top rather than splaying apart there — see CrackedBrandTile in
//     equifaxShared. The V2 brief gives both the gaps (14 at the foot, 6 at the
//     top) and a hinge at the outer feet, which are opposite pictures; the gaps
//     win, because in this cut the records leave through the foot and the break
//     has to be widest where they leave it.
//   * THE SPILL IS DRAWN UNDER THE CHART LINE, which is the V2 brief's own
//     instruction for the heap, applied to the cards in flight as well. It is
//     also what keeps the promise that the arcs never cut the amber leg: the
//     two left-hand slots of the bottom row sit just past the foot of the
//     cliff, and the cards that fill them pass BEHIND the leg, not across it.
//   * THE HEAP'S ROTATIONS AND ITS 1 px OF SLOP ARE THE ONLY HASHED NUMBERS IN
//     THE CUT. Rows, pitches and the axis are composed, per V2_COMMON.
//   * A CARD'S TUMBLE LANDS ON ITS SLOT'S ANGLE — see spillRecords. Written as
//     the brief has it, "0 to rot + 180 * hash", a record would come to rest up
//     to a half turn from the angle the heap chose for it, i.e. upside down.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// V2 · THE BREAK, AND WHAT COMES OUT OF IT.
//
// The tag lands INTACT at f30 — up to that frame this file is V1 exactly, and
// `CrackedBrandTile` at open 0 renders the single `BrandTile` path, so there is
// not even a seam to see. Then it breaks: `open` 0 -> 1 over f30 -> f38, on
// `camEase(u, 0.8)` so the snap is in the first half of the move and the halves
// settle into their lean. THE CRACK OPENING IS "THAT MOMENT". It is the only
// thing that happens to the tag — there is no ink click on it, because the
// cut's one accent is the amber leg the price is laying at exactly the same
// time, and two clicks on one beat is two moments.
//
// Eight frames is the held breath's own length: the camera is dead still f30-38
// while the price falls the cliff and the tile splits, and the frame the camera
// lets go is the frame the break is fully open.
// ---------------------------------------------------------------------------
export const CRACK_F0 = 30; // "moment": the tag has landed, the tile gives way
export const CRACK_F1 = 38; // fully open, the frame the camera lets go
export const CRACK_WARP = 0.8;
export const CRACK_SEED = 11;
export const crackOpen = (f: number) =>
  camEase(clamp01((f - CRACK_F0) / (CRACK_F1 - CRACK_F0)), CRACK_WARP);

// WHERE THE RECORDS COME OUT: the foot of the crack, in world px. The crack is
// cut in the tile's own space and the tile's top-left is (TAG_X1 - TAG.w / 2,
// TAG_Y1 - TAG.h / 2), so the foot is on the tag's bottom edge — which is the
// line at the top of the cliff. Records fall out from under the tile, exactly
// where the dots used to.
export const CRACK_FOOT_LOCAL = crackFoot(TAG.w, TAG.h, CRACK_SEED);
export const SPILL_FROM = {
  x: TAG_X1 - TAG.w / 2 + CRACK_FOOT_LOCAL.x, // 658.19
  y: TAG_Y1 - TAG.h / 2 + CRACK_FOOT_LOCAL.y, // 889.68 = PY(I_BREACH)
};

// THE RECORDS ARE TAG-SCALE — AND REV 1 MAKES THEM BIGGER. The tag is half the
// crowd tile (150 x 48). V2 drew its records a third of the default card, 12 x
// 16 world px, and Fable's review of the preview is the reason this revision
// exists: at the moment itself — f34-f45, the frames the viewer is closest to —
// a 12 px card at k 2.28 is 27 screen px wide and a stream of them three frames
// apart reads as a TRICKLE OF CHIPS, not as files coming out of a broken tile.
// So 17 x 22: a card a bit over a third of the tag's own HEIGHT (22 against 48),
// which at K_ROOM 2.28 is 39 x 50 screen px through the drop — a card whose
// person and two lines are legible — and 17 x 22 at k 1.00, still a pale chip in
// the pile at the notch but a chip with a shape. The three numbers move
// together: a bigger card at the same rate would leave gaps in the stream, so
// the rate halves (3 -> 2) and the count goes 16 -> 22, which is exactly the six
// rows 6+5+4+3+2+2 the heap fills. Nothing else in the cut changes.
export const REC = { w: 17, h: 22 };
export const SPILL_N = 22;
export const SPILL_START = 34; // four frames into the break, when there is a gap
export const SPILL_RATE = 2;
export const SPILL_FLIGHT_2 = 22;
export const SPILL_PEAK_2 = 4; // world px; the arc never lifts above the break
export const SPILL_SEED = 37;

// THE HEAP IS AT THE FOOT OF THE NOTCH, centred on the low and standing on it:
// `heapSlots` with the card's own pitches, 22 cards in rows of 6, 5, 4, 3, 2, 2,
// the base at the low's own y. It is the only composed layout this cut has and
// it is the thing that has to read at k 1.00 — a small pale pile under a small
// pale tag, on a line that has gone on to triple. At 17 x 22 the pile is 83
// world px wide and 90 tall (row pitch 0.62 x 22 = 13.64, six rows), so its top
// sits at y 942 — 53 px below the crack's foot at 890 and clear of it; the 2018
// line runs through its upper rows, and the line is drawn ON TOP, which is the
// V2 brief's own instruction: the paper is on the floor under the chart.
export const HEAP_CX = PX(I_LOW); // 584.15
export const HEAP_BASE = PY(I_LOW); // 1032.68
export const HEAP_MAX_ROW_2 = 6;
export const HEAP_SEED = 5;
export const HEAP = heapSlots(SPILL_N, HEAP_CX, HEAP_BASE, HEAP_SEED, {
  w: REC.w,
  h: REC.h,
  maxRow: HEAP_MAX_ROW_2,
});
...
const HardToFindV2: React.FC<Props> = ({
  ink,
  accent,
  backgroundBase,
  parallax,
  ...
  // A breach: the tile gives way and the records inside it fall out of the
  // break and heap at the foot of the cliff. Drawn UNDER the line and the tag,
  // so they come out from beneath the tile rather than off its face — and so
  // the amber leg, which the price is laying down the same cliff at the same
  // time, is never crossed by a card: the cards pass behind it.
  const open = crackOpen(frame);
  const cards = spillRecords({
    frame,
    start: SPILL_START,
    rate: SPILL_RATE,
    from: SPILL_FROM,
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* the records: in the air, and heaped at the foot of the notch.
                Under the line and under the tag — a card lies ON the sheet. */}
            {cards.map((c) => (
              <RecordCard
                key={c.key}
                x={c.x}
