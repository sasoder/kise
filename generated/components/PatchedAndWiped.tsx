...
//      clockwise, around the dead hub AND its crack: world
//      (470, 690) to (640, 830). Closes at f28 with the
//      4-frame click on close. It stays
//                                      — "this crash"            f17-32
//   3. TRACED. An ink line (stroke 3, OP_READ, head-led with
//      the field's white tip) traces the exploit route ON TOP
//      of the dead accent route, from the crack's midpoint at
//      f49, at 24 world px/frame — the calm cap at k 1.00. It
//      is 1,392 px along the 1,983 at "exploit" f107 and runs
//      off the top of the frame at f98.9, completing off-frame
//      at f131.6. OpenAI following the wire the agents laid
//                                      — "figured out that the
//                                         agents had built this
//                                         exploit"               f44-107
//   4. SEALED AND SEVERED.
//      f142-158: the crack SEALS. The two arc ends grow toward
//        each other in ink, head-led from 20 deg and from
//        60 deg, meeting at the gap's midpoint 40 deg at f156
//        with a 4-frame click on the ring. The ring is a full
//        circle again: the sealed segment is ink, the rest of
//        it stays dead accent
//                                      — "patched the"           f142-158
//      f159-175: the exploit route is ERASED from the crack
//        outward. The ink trace and the dead accent route
//        beneath it un-draw together behind one ink tip, and
//        by f175 there is no route. The internet ring keeps
//        its beads — that is history — but its right edge is
//        bare
//                                      — "relevant vulnerability" f159-175
//   5. THE PULL-BACK. Move 2, and nothing else happens
//                                      — "and as a result"       f172-196
//   6. THE LOOK ENDS. The look-box fades out over 8 frames.
//      OpenAI is done looking; it did not notice what comes
//      next                            — "inadvertently"         f216-224
//   7. THE WIPE. The dead accent DRAINS OUT OF THE PLUMBING
//      FROM THE HUB OUTWARD, un-drawing on cut 7's own
//      conversion geometry with the fronts reversed: out of
//      the parcel's centre (f230-236), then the ring
//      (f236-244), then along the four remaining lines at
//      45 world px/frame (f244 on) — the route is already
//      gone. Each front carries a white tip. What is revealed
//      behind it is CLEAN INK PLUMBING at OP_READ: rail,
//      pipes, ring with the sealed crack, parcel — exactly
//      cut 3's package manager, fresh. Every ringed agent's
//      ring fades out over 8 frames from f262: the agents
//      that had the board are just agents again
//                                      — "wiped the agents'
//                                         message board in the"  f230-270
//   8. HELD: fresh white plumbing, no traffic anywhere, no
...
//     (the brief's "our box 553-789" was an estimate; 553 is the top wall and
//      973 the bottom, and both are reported here rather than quietly kept.)
//
//   MOVE 1's REST, k 1.00 / c 760 / cx 540 — the close-up of the crash.
...
//
//   MOVE 2's REST, k 0.45 / c 180 / cx 540 — the reach of the wipe.
...
//     frame — the brief expected them in it; the boxes either side do come in
//     at the edges (world x -660..-410 and 1490..1740) and the rail crosses
//     both. Reported rather than fixed by widening: the shot the brief asks
//     for is the mark, the ring, our box, the rail to both edges and row 1,
//     and k 0.45 gives all five.
//
// THE CAMERA — two moves, one per act, each eased per frame by `camMove` and
// damped by `runCamera`, with true holds between and `sway` throughout.
//   MOVE 1, "OpenAI noticed this crash": warp 0.72, k 0.60 -> 1.00, content
//     centre 180 -> 760, cx 540 throughout. KEYS f0 -> f10, NOT the briefed
//     f2 -> f14: at f2->f14 the damper is still 3.7% short of k 1.00 at "this
//     crash" f17 and closing at 1.1% a frame, which is above the 1%/frame that
//     reads as movement — the box would have drawn while the shot was still
//     arriving. Measured on the same damper for f0->f10: k 0.9894 at f17
//     (1.1% short, closing at 0.57% a frame — under the readable floor),
//     0.9997 at f22, 1.0000 from f26. Peak zoom rate 4.0%/frame, so it is
//     still a clear push. Reported k: f14 0.9648, f17 0.9894, f22 0.9997,
//     f26 1.0004.
//   MOVE 2, "and as a result": keys f172 -> f186, warp 0.72, k 1.00 -> 0.45,
//     content centre 760 -> 180, cx 540 throughout. Reported k: f186 0.5517,
//     f196 0.4512, f216 0.4500 — settled to 0.3% by f196 and to four decimal
//     places by f200, sixteen frames clear of "inadvertently" f216.
...
// each leaving ink at OP_READ behind it.
//   the parcel, f230 -> f236, cut 7's own three stages compressed 8 frames into
//     6 (the clock runs 1.333x), so the shape of the drain is the shape of the
//     conversion played backwards in colour:
//       three inner edges, centre outward, 26.00 px   f230.0 -> f232.25
//                                                     11.556 px/frame
//       the hexagon, three two-edge runs, 51.554 px   f232.25 -> f234.5
//                                                     22.913 px/frame
//       the strap, 25.554 px                          f234.5 -> f236.0
//                                                     17.036 px/frame
//   the ring, f236 -> f244, both arcs, seeded at the top (360 deg) and the
//     bottom (180 deg) and run to the crack ends at 20 and 60 deg; four fronts,
//     longest 140 deg = 97.738 px                     12.217 px/frame
//                                                     (17.500 deg/frame)
//   then out along the four remaining lines at once, at DRAIN_V 45 world
//   px/frame — 20.25 screen px/frame at k 0.45, inside the cap:
//     pipe up     (540, 720) -> y  410    310 px  f244.0 -> f250.9, and it
//                                                 STOPS in our box's floor
//     pipe down   (540, 800) -> y 1110    310 px  f244.0 -> f250.9, and it
//                                                 STOPS in row 1's ceiling
//     rail left   (500, 760) -> x -2260  2760 px  f244.0; past the k 0.45 left
//                                                 frame edge (x -660) at
//                                                 f269.8, and off the end of
//                                                 the world at f305.3
//     rail right  (580, 760) -> x  3340  2760 px  f244.0; past the k 0.45
//                                                 right frame edge (x 1740) at
//                                                 f269.8, same end at f305.3
//     the exploit route: NOT drained. It was erased at f159-175 and there is
//       nothing left of it to drain.
//   So the frame is clean of accent at f269.8, seven frames before "process"
//   f276 and thirty-five before the last frame. The brief's "by f262" is the
//   word, not the arithmetic: at f262 the two rail fronts are at screen x 157.5
//   and 922.5, still inside the frame and still running. 45 px/frame is the
//   briefed speed and it is kept; what is reported is where it actually lands.
...
//   the look-box's drawing tip   56.364 world px/f  @ k 1.00  = 56.4 screen
...
//       it, so consecutive frames overlap however fast the tip goes.
//   the exploit trace's tip      24.000 world px/f  @ k 1.00  = 24.0 screen  OK
//   the seal's two tips           0.997 world px/f  @ k 1.00  =  1.0 screen  OK
//   the erase's tip             123.913 world px/f  @ k 1.00  = 123.9 screen
//     — the same exemption, in reverse: the stroke is on screen AHEAD of it.
//       The briefed 60 px/frame is not kept, and the reason is the camera: at
//       60 the erase would still be running at f192, and the far end of the
//       route (the run along y -400 into the internet ring) comes into frame
//       at f179 as move 2 opens out, so the last 460 px would have been erased
//       in plain sight during a beat the brief reserves for the camera alone.
//       1,982.6 px over f159 -> f175 is the brief's own stated outcome — "by
//       f175 there is no route" — and that fixes the speed at 123.913.
//   the parcel drain's tips  11.556 / 22.913 / 17.036 world px/f @ k 0.45
//                                                    = 5.2 / 10.3 / 7.7  OK
//   the ring drain's tips        12.217 world px/f  @ k 0.45  =  5.5 screen  OK
...
export const M2_F0 = 172;
export const M2_F1 = 186;
...
// ---------------------------------------------------------------------------
// THE DRAIN. Cut 7's strokes, its geometry and its stage velocities, with the
// fronts running the same way and the colours the other way round: ink is dashed
// ON from the seed to the front, the dead accent is dashed OFF over exactly the
// same span. ALL_STROKES is cut 7's array — 0-2 the parcel's inner edges, 3-5
// its hexagon runs, 6 its strap, 7-10 the ring's four arcs, 11-14 the four lines
// out of the ring — and only the clock is this piece's.
// ---------------------------------------------------------------------------
export const DRAIN_PKG_F0 = 230; // "wiped the"
export const DRAIN_PKG_F1 = 236;
export const DRAIN_RING_F0 = 236;
export const DRAIN_RING_F1 = 244;
export const DRAIN_LINE_F0 = 244;
export const DRAIN_V = 45; // world px/frame, 20.25 screen px/frame at k 0.45
// cut 7's own conversion clock, compressed into this piece's six parcel frames
const pkgClock = (f: number) =>
  CONV_F0 + ((f - DRAIN_PKG_F0) * (PKG_F1 - CONV_F0)) / (DRAIN_PKG_F1 - DRAIN_PKG_F0);

export const drainFront = (i: number, f: number): number => {
  if (i < 3) return (pkgClock(f) - CONV_F0) * V_PKG_EDGE;
  if (i < 6) return (pkgClock(f) - PKG_EDGE_F1) * V_PKG_HEX;
  if (i === 6) return (pkgClock(f) - PKG_HEX_F1) * V_PKG_STRAP;
  if (i < 11) return (f - DRAIN_RING_F0) * V_RING;
  return (f - DRAIN_LINE_F0) * DRAIN_V;
};
...
// ---------------------------------------------------------------------------
export const LOOK = { x0: 470, y0: 690, w: 170, h: 140 }; // world; the hub AND its crack
export const LOOK_PERIM = 2 * (LOOK.w + LOOK.h); // 620
...
export const V_LOOK = LOOK_PERIM / (LOOK_F1 - LOOK_F0); // 56.364 world px/frame

// The perimeter walked clockwise from the top-left corner, for the white tip.
// The corner radius is 2 world px on a 170 x 140 box, so the rectangle's own
...
export const V_TRACE = 24; // world px/frame, the calm cap at k 1.00
export const ERASE_F0 = 159; // "relevant"
export const ERASE_F1 = 175; // "vulnerability" ends; there is no route
export const V_ERASE = EXPLOIT_STROKE.len / (ERASE_F1 - ERASE_F0); // 123.913
...
export const SEAL_F1 = 156; // met at the midpoint
...
export const V_SEAL = SEAL_STROKES[0].len / (SEAL_F1 - SEAL_F0); // 0.997 px/frame
...
const PatchedAndWiped: React.FC<Props> = ({
  ...
  // -- the drain -------------------------------------------------------------
  // One front per stroke. Behind it is ink; ahead of it is what the crash left.
  const drain = ALL_STROKES.map((s, i) =>
    Math.max(0, Math.min(s.len, frame < DRAIN_PKG_F0 ? 0 : drainFront(i, frame))),
  );
  const drainTips = ALL_STROKES.map((s, i) =>
  ...
  const drainedInk = (s: Stroke, i: number) => (
    <Span
      key={`i${s.key}`}
      d={s.d}
      a={0}
  ...
  const deadAccent = (s: Stroke, i: number) => (
    <Span
      key={`a${s.key}`}
      d={s.d}
      a={drain[i]}
      ...
      >
        ...
        >
          ...
          >
            ...
            <g style={{ filter: icon }}>
              {ALL_STROKES.slice(HUB_STROKES.length).map((s, i) =>
                drainedInk(s, HUB_STROKES.length + i),
              )}
              {ALL_STROKES.slice(HUB_STROKES.length).map((s, i) =>
                deadAccent(s, HUB_STROKES.length + i),
              )}
            </g>

            {/* THE HUB: the ring the crash left cracked and the parcel inside
                it, dead accent until the wipe takes it back to ink. The seal is
                its own ink arc: it is what OpenAI put there, not what drained. */}
            <g style={{ filter: icon }}>
              {HUB_STROKES.map((s, i) => drainedInk(s, i))}
              {HUB_STROKES.map((s, i) => deadAccent(s, i))}
              {SEAL_STROKES.map((s) => (
                <Span
                  key={s.key}
                  d={s.d}
        ...
        >
          ...
          >
            <g style={{ filter: icon }}>
              ...
              {/* the drain's own white tips — the field's tip, at exactly the
                  radius a packet head would have, so the wipe and the traffic
                  that used to run here read as one language */}
              {drainTips.map((t) => (
