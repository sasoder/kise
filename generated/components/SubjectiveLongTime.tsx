...
// A short ink ruler beside the spine: three days, and they never get longer.
const RULER = { x: 320, y0: 820, y1: 1180, ticks: [820, 940, 1060, 1180] };
...
const CAM_CY = [1560, 1560, 1560, 1500, 1650, 1650, 1050, 1050, 1050, 1250, 1250];
const CAM_K = [0.74, 0.74, 0.78, 0.85, 0.85, 0.85, 1.0, 1.0, 1.0, 0.72, 0.72];
...
const SubjectiveLongTime: React.FC<Props> = ({
  ink,
  accent,
  backgroundBase,
  backgroundSrc,
  ...
  const playY = barEnd + (PLAYHEAD_REST - barEnd) * scrub;
  ...
  // -- the human ruler ------------------------------------------------------
  const rulerDraw = interpolate(frame, [beats.longTime, beats.longTime + 12], [0, 1], {
    ...clamp,
    easing: Easing.out(Easing.cubic),
  });
  const rulerGone = interpolate(frame, [beats.afterTheyHad, beats.afterTheyHad + 12], [1, 0], clamp);
  const dayDot = interpolate(frame, [beats.atLeast, beats.ofTheAis], [RULER.ticks[0], RULER.ticks[1]], {
    ...clamp,
    easing: Easing.inOut(Easing.quad),
  });
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* the human ruler */}
            {rulerDraw > 0 && rulerGone > 0 ? (
              <g opacity={rulerGone}>
                <line
                  x1={RULER.x}
                  y1={RULER.y0}
                  x2={RULER.x}
                  y2={RULER.y0 + (RULER.y1 - RULER.y0) * rulerDraw}
                  stroke={ink}
                  strokeWidth={11}
                  strokeLinecap="round"
                />
                {RULER.ticks.map((y, i) => {
                  const on = interpolate(RULER.y0 + (RULER.y1 - RULER.y0) * rulerDraw - y, [-2, 10], [0, 1], clamp);
                  return <line key={`r${i}`} x1={RULER.x - 26} y1={y} x2={RULER.x + 26} y2={y} stroke={ink} strokeWidth={8} strokeLinecap="round" opacity={on} />;
                })}
                {frame >= beats.atLeast - 6 ? (
                  <circle cx={RULER.x} cy={dayDot} r={16 * interpolate(frame, [beats.atLeast - 6, beats.atLeast], [0, 1], clamp)} fill={ink} />
                ) : null}
              </g>
            ) : null}
...
export default SubjectiveLongTime;
