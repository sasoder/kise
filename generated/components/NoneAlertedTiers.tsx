...
const NoneAlertedTiers: React.FC<Props> = ({
  ink,
  accent,
  backgroundBase,
  backgroundSrc,
  ...
  const agents: Agent[] = CROWD_POS.map((p, i) => {
    ...
    if (j !== undefined) {
      ...
      x = x + (rp.x - x) * rowLift;
      y = y + (rp.y - y) * rowLift;
      hold = interpolate(frame, [beats.restrain + j * 2, beats.restrain + j * 2 + 8], [0, 1], { ...clamp, easing: Easing.out(Easing.cubic) });
    }
  ...
  for (let j = 0; j < threads; j++) {
    ...
    threadEls.push({ key: j, x1: A.x, y1: A.y, x2: A.x + (B.x - A.x) * drawn, y2: A.y + (B.y - A.y) * drawn, op: 0.85 * fade });
  }
                ...
                <g key={i}>
                  ...
                  {g.hold > 0 ? (
                    <circle
                      cx={g.x}
                      cy={g.y}
                      r={r + 7 + 3 * constraintPulse}
                      fill="none"
                      stroke={ink}
                      strokeWidth={3.5 + 2 * constraintPulse}
                      pathLength={1000}
                      strokeDasharray={1000}
                      strokeDashoffset={1000 * (1 - g.hold)}
...
export default NoneAlertedTiers;
