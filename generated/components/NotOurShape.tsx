...
const HEAD = {x: CX, y: 668, r: 120};
const BODY = {x: CX, y: 1216, rx: 238, ry: 424, top: 800, bottom: 1216};
...
const humanPts = (() => {
  ...
  while (pts.length < N && k < N * 40) {
    const x = CX - 300 + hash(k, 3) * 600;
    const y = 530 + hash(k, 4) * 700;
    k += 1;
    if (!inHuman(x, y)) {
      continue;
    }
    pts.push({x, y, a: Math.atan2(y - 940, x - CX)});
    i += 1;
  }
  return pts;
})();
...
export type NotOurShapeProps = z.infer<typeof schema>;

export const defaultProps: NotOurShapeProps = schema.parse({
  ink: '#FFFFFF',
  accent: '#48D9FF',
...
const NotOurShape: React.FC<NotOurShapeProps> = ({
  ink,
  accent,
  shadow,
  dotMin,
  ...
  const lens = {
    x: LENS.x + (1 - glassIn) * 268 + 4 * Math.sin(frame / 31),
    y: LENS.y + (1 - glassIn) * 256 + 3.5 * Math.cos(frame / 26),
  };
    ...
    <AbsoluteFill>
        ...
        <g style={{filter: `drop-shadow(0 2px 6px ${shadow})`}}>
            ...
            return (
              <g key={`l${j}`}>
                <line
                  ...
                  strokeLinecap="round"
                  opacity={alive * (0.34 - 0.16 * recede + 0.58 * u)}
                />
                <circle
                  cx={a.x + (ex - a.x) * t}
                  cy={a.y + (ey - a.y) * t}
          ...
          {dots.map((d, i) => (
            <circle
              ...
              r={d.r}
              fill={accent}
              opacity={0.5 - 0.24 * recede + 0.55 * d.u}
            />
          ))}
      ...
      <Img
        ...
        style={{
          ...
          width: glassSize,
          height: glassSize,
          filter: `brightness(0) invert(1) drop-shadow(0 2px 6px ${shadow})`,
