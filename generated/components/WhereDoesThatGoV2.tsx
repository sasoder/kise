...
import { z } from "zod";
import { CAM_LIFT, camEase, clamp, clamp01, smoothstep, sway } from "./fieldShared";
import { CX_OPEN_CFE, CY_OPEN_BAR_CFE, cfeCameraV2 } from "./FeedEverything";
import {
  BLACK,
  BLUE,
  CFE_CAM_CX,
...
// ---------------------------------------------------------------------------
// V2 — THE USER'S TWO CHANGES (everything else is the delivered
// `WhereDoesThatGo`, which stays untouched; same 179 frames, same 21.20 s in).
//
// 1. THIS CUT'S f0 IS CUT 1'S LAST FRAME, pixel for pixel. The camera is cut
//    1's own damped camera continued on cut 1's clock (g = f + JOIN_F, JOIN_F =
//    CFE_DURATION - 1 = 120): its keys up to g 120 are cut 1's (`CFE_CAM_*`),
//    so the damper arrives with cut 1's position AND velocity and cut 1's creep
//    keeps running to its own end (our f16) — f0 is not a dead stop. Asserted
//    at module load against `cfeCameraV2` (cut 1 f118-120) and CFE_RESOLVED.
//    The paper drift and the hand's sway run on the same clock (frame + 120)
//    and the paper's parallax rest is cut 1's (CX_OPEN_CFE, CY_OPEN_BAR_CFE).
//    Proved with full-res stills: cut 1 f120 vs this cut f0, max abs pixel
//    difference 0, mean 0, 0 of 2,073,600 pixels differ.
//    THE OPENING MOVE: "it knows" (7-17) plays on the whole ChatGPT window;
//    from f6 glide 1 is a PUSH-IN (f6-42, warp 1.0) from cut 1's creep (k 1.02,
//    the window on 835) onto the typing: k -> 1.165, cx -> 553, content centre
//    -> 1080 — the same target as V1, so f46 on is V1's camera. The header
//    leaves through the top on the way in (below y 200 until f15). Window
//    centre's screen |dv| peaks at 1.06 px/f^2 (f43); handover to glide 2 at
//    f46 is C1 (|dv| 0.70 there). Typed line inside screen x 103..985 on every
//    frame.
//      f    k        cx       cy
//      0    1.0132   540.00   958.37    = CFE_RESOLVED, cut 1's creep running
//      7    1.0167   540.00   958.00    "it"
//      10   1.0189   540.09   959.44    "knows"
//      17   1.0354   541.42   982.46    "how": typing starts
//      28   1.0915   546.41  1070.32
//      33   1.1200   548.97  1115.68    "cottage"
//      44   1.1608   552.62  1180.57    the line is whole
//      48   1.1644   553.13  1185.73    SEND (from here as V1, +-0.01 k)
//
// 2. THE SPOTLIGHT on "Where does that go?": one uniform black layer over the
//    whole world at 0 -> DIM_MAX 0.5 on one ease-out (quad) ramp f127 -> f141
//    (0.069 at f128, 0.459 at "does" f137, 0.500 from f141), held to the last
//    frame. White ink under it reads mid-grey (#808080), the paper dark grey.
//    Above it at full brightness: the copy, its hard shadow and the whole crown
//    (from the peek on, so no colour ever switches from dim to bright), masked
//    by the original pill's footprint (pill + its shadow), so they still come
//    out from BEHIND the (now dimmed) original. No glow, no scale, no vignette.
//
...
// ---------------------------------------------------------------------------
// THE CAMERA — see the header. V2: THIS CUT'S f0 IS CUT 1'S LAST FRAME. The
// camera is cut 1's own damped camera, continued: one track on cut 1's clock
// (g = f + JOIN_F) whose keys up to g = JOIN_F are cut 1's keys exactly, so
// the damper arrives at our f0 with cut 1's position AND velocity (its creep
// is still running, and keeps running to its own end, our f16). This cut's
// four glides are eased deltas added on top of that base, from our f6 on.
// cy = centre + CAM_LIFT / k throughout. Asserted at module load against cut
// 1's `cfeCameraV2` on its last three frames.
// ---------------------------------------------------------------------------
/** Cut 1's last frame; this cut's f0 is that frame, on cut 1's clock. */
export const JOIN_F = CFE_DURATION - 1; // 120
/** Cut 1's camera keys, sampled on cut 1's clock (its creep runs to g 136). */
const baseAt = (g: number) => {
  const k = interpolate(g, CFE_CAM_F, CFE_CAM_K, clamp);
  const cy = interpolate(g, CFE_CAM_F, CFE_CAM_CY, clamp);
  const cx = interpolate(g, CFE_CAM_F, CFE_CAM_CX, clamp);
  return { k, cx, c: cy - CAM_LIFT / k };
};
/** Where cut 1's creep ends (k 1.02, the window on 835). */
export const BASE_END = baseAt(CFE_CAM_F[CFE_CAM_F.length - 1]);
...
export const T1 = { k: 1.165, cx: 553, c: 1080 }; // the typed line in the band, header off the top
...
export const GLIDES: Glide[] = [
  // G1, the PUSH-IN: from the whole window (cut 1's creep) onto the typing
  { f0: 6, f1: 42, warp: 1, k: T1.k - BASE_END.k, cx: T1.cx - BASE_END.cx, c: T1.c - BASE_END.c },
...
const TRACK = (() => {
  ...
  const CY: number[] = [];
  const CX: number[] = [];
  for (let g = 0; g <= JOIN_F + DURATION + 24; g++) {
    const b = baseAt(g);
    let k = b.k;
    let cx = b.cx;
    let c = b.c;
    for (const gl of GLIDES) {
      const e = camEase((g - JOIN_F - gl.f0) / (gl.f1 - gl.f0), gl.warp);
      k += gl.k * e;
      cx += gl.cx * e;
      c += gl.c * e;
    }
...
/** The camera at THIS cut's frame f (no sway). */
export const cwgCamera = (f: number) =>
  runCamera2(f + JOIN_F, CWG_CAM_F, CWG_CAM_CY, CWG_CAM_CX, CWG_CAM_K);
// THE JOIN, asserted: our f-2 .. f0 are cut 1's f118 .. f120 (so position and
// velocity both match), and f0 is CFE_RESOLVED.
{
  let err = 0;
  for (let f = -2; f <= 0; f++) {
    const a = cwgCamera(f);
    const b = cfeCameraV2(JOIN_F + f);
    err = Math.max(err, Math.abs(a.k - b.k), Math.abs(a.cx - b.cx), Math.abs(a.cy - b.cy));
  }
  const z = cwgCamera(0);
  err = Math.max(
    err,
    Math.abs(z.k - CFE_RESOLVED.k),
    Math.abs(z.cx - CFE_RESOLVED.cx),
    Math.abs(z.cy - CFE_RESOLVED.cy),
  );
  if (err > 1e-6) {
    throw new Error(`WhereDoesThatGoV2: the join camera drifts from cut 1's last frame (by ${err})`);
  }
}
// The paper's parallax rest and the hand's sway are cut 1's, on cut 1's clock.
const REST = { cx: CX_OPEN_CFE, cy: CY_OPEN_BAR_CFE };
export const swayAt = (f: number) => sway(f + JOIN_F);

// ---------------------------------------------------------------------------
// THE SPOTLIGHT — "Where does that go?": one uniform black layer over the
// whole world at 0 -> DIM_MAX on one ease-out ramp, f127 -> f141, held to the
// last frame. Only the copy, its hard shadow and its carried crown are drawn
// ABOVE it, at full brightness, masked by the original pill's footprint (pill
// + its shadow) so they still come out from BEHIND the original.
// ---------------------------------------------------------------------------
export const DIM_F0 = 127;
export const DIM_F1 = 141;
export const DIM_MAX = 0.5;
const EASE_DIM = Easing.out(Easing.quad);
export const dimAt = (f: number) => DIM_MAX * EASE_DIM(clamp01((f - DIM_F0) / (DIM_F1 - DIM_F0)));
...
const COPY_CLIP = "cwg2-copy-clip";
const FREE_MASK = "cwg2-outside-original";

const WhereDoesThatGoV2: React.FC<Props> = ({ sway: withSway, stampTime, stampDay }) => {
  const frame = useCurrentFrame();
  const cam = cwgCamera(frame);
  const d = withSway ? swayAt(frame) : { dx: 0, dy: 0 };
  ...
  return (
    <World frame={frame + JOIN_F} cam={{ cx: cam.cx + d.dx, cy: cam.cy + d.dy, k: cam.k }} rest={REST}>
      <defs>
        <clipPath id={COPY_CLIP}>
          <rect x={-4000} y={-9000} width={9000} height={9000 + PILL_Y1} />
        </clipPath>
        {/* everything OUTSIDE the original pill's footprint (pill + shadow):
            the bright layer comes out from behind the original */}
        <mask id={FREE_MASK} maskUnits="userSpaceOnUse" x={-5000} y={-10000} width={11000} height={20000}>
          <rect x={-5000} y={-10000} width={11000} height={20000} fill={WHITE} />
          <rect x={PILL_X0} y={PILL_Y0} width={PILL_W} height={PILL_H} rx={PILL_R} fill={BLACK} />
          <rect
            x={PILL_X0 + SHADOW}
            y={PILL_Y0 + SHADOW}
            width={PILL_W}
      ...
      {/* ---- THE WORLD: everything that dims ---- */}
      <LogWindow log={LOG} frame={frame} idPrefix="cwg2" overExchange={stamp} />

      {landed ? (
        <g>
          {/* the original, redrawn as it has been since it landed */}
          <rect
            x={PILL_X0 + SHADOW}
            y={PILL_Y0 + SHADOW}
            width={PILL_W}
      ...
      {/* ---- THE SPOTLIGHT: one uniform black layer over the whole world ---- */}
      {dim > 0 ? (
        <rect x={-5000} y={-10000} width={11000} height={20000} fill={BLACK} opacity={dim} />
      ) : null}

      {/* ---- THE BRIGHT LAYER: the copy and its crown, never dimmed ---- */}
      {landed ? (
        <g mask={`url(#${FREE_MASK})`}>
          <g clipPath={`url(#${COPY_CLIP})`}>
            ...
            {/* the copy: exactly the pill, its text */}
            {D > 0 ? (
...
export default WhereDoesThatGoV2;
