...
// Stable per-dot scatter — same value every frame, so nothing flickers.
const hash = (i: number) => {
  const v = Math.sin(i * 12.9898 + 78.233) * 43758.5453;
  return v - Math.floor(v);
};
const sgn = (i: number) => hash(i) * 2 - 1;
...
// Two land on "government", two on "policy", two on "history", each pair
// further out and lower than the last; the outermost run off the frame edges.
// ---------------------------------------------------------------------------
type Heap = {
  x: number;
  count: number;
...
const HEAPS: Heap[] = [
  { x: 900, count: 16, cols: 4, opacity: 0.85, at: "government", lag: 0 },
  { x: 1700, count: 11, cols: 4, opacity: 0.76, at: "government", lag: 4 },
  { x: 660, count: 6, cols: 3, opacity: 0.62, at: "policy", lag: 0 },
  { x: 1940, count: 6, cols: 3, opacity: 0.55, at: "policy", lag: 4 },
  { x: 480, count: 2, cols: 2, opacity: 0.42, at: "history", lag: -5 },
  { x: 2120, count: 2, cols: 2, opacity: 0.36, at: "history", lag: -3 },
];
...
// ---------------------------------------------------------------------------
const CAM_F = [0, 24, 50, 70, 84, 94, 102, 110, 118, DURATION];
const CAM_CY = [4496, 4495, 4488, 4474, 4452, 4420, 4340, 4180, 3992, 3992];
const CAM_K = [1.25, 1.25, 1.23, 1.18, 1.1, 1.0, 0.88, 0.75, 0.636, 0.636];

// Stiffer than the long-form cut: a 17-frame settle is an eighth of this piece.
const CAM_STIFF = 0.14;
const CAM_DAMP = 0.58; // zeta ~0.78, settles in about 14 frames
...
// The second mark takes the ridge's own step outward — smaller and dimmer by
// the same ratio the heaps themselves diminish by — so the pair reinforces the
// recession into history instead of reading as two separate labels.
const GOV2_X = 1700;
const GOV2_Y = 4552;
const GOV2_STEP = 0.78;
const GOV2_FADE = 0.72;
...
const MoreBusinessesColumn: React.FC<Props> = ({
  ink,
  accent,
  backgroundBase,
  backgroundSrc,
  ...
  const gov2Y = GOV2_Y + 22 * (1 - gov2In);
  ...
  const ambientDim = interpolate(frame, [beats.slam, beats.businesses], [0.34, 0.1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
    ...
    <AbsoluteFill style={{ backgroundColor: backgroundBase }}>
      ...
      >
        ...
        >
          ...
          >
            <rect x={60} y={FLOOR_Y} width={2480} height={5} fill={ink} opacity={floorOpacity} />
            ...
            {marks && gov2In > 0 ? (
              <g
                transform={`translate(${GOV2_X} ${gov2Y}) scale(${(govSize * GOV2_STEP) / 24}) translate(-12 -12)`}
                opacity={govOpacity * GOV2_FADE * gov2In}
              >
                <path d={GOV_D} fill={ink} />
              </g>
            ) : null}
            ...
            {Array.from({ length: upto }, (_, i) => {
              ...
              return (
                <circle
                  ...
                  cy={sy}
                  r={r * pop}
                  fill={interpolateColors(Math.min(age, 2), [0, 2], ["#FFD98A", accent])}
                />
              );
            })}
...
export default MoreBusinessesColumn;
