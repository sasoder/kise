...
export type NotOurShapeGridEnhancedProps = z.infer<typeof schema>;

export const defaultProps: NotOurShapeGridEnhancedProps = schema.parse({
  ink: '#FFFFFF',
  accent: '#48D9FF',
...
const NotOurShapeGridEnhanced: React.FC<NotOurShapeGridEnhancedProps> = ({
  ink,
  accent,
  shadow,
  dotMin,
  ...
  // It draws breath before it is pressed: a small contraction that releases
  // into the move, so the press has somewhere to come from.
  const breath = interpolate(frame, [beats.avoid + 2, beats.avoid + 14, beats.anthro - 2], [0, 1, 0], {
    easing: Easing.inOut(Easing.quad),
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  // ...and lands with a settle rather than a stop.
  const press = interpolate(frame, [beats.avoid + 8, beats.anthro + 1], [0, 1], {
    easing: Easing.bezier(0.34, 1.18, 0.4, 1),
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  ...
  const glassIn =
    frame < beats.worth
      ? 0
      : spring({frame: frame - beats.worth, fps, config: {damping: 11, stiffness: 105}});
  ...
  const dots = own.map((p0, i) => {
    const p = {
      x: CX + (p0.x - CX) * ballK * (1 - 0.05 * breath),
      y: CY + (p0.y - CY) * ballK * (1 - 0.05 * breath),
    };
    const t = target[i];
    ...
    const held = press * (1 - burst);
    const heldC = clamp01(held);
    ...
    return {x: px, y: py, r: rr, u: clamp01(0.58 * resolve + 0.62 * k), held: heldC};
  });
...
export default NotOurShapeGridEnhanced;
