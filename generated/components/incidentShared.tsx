...
// ---------------------------------------------------------------------------
// incidentShared — FILM A of `Noam_Airgapping`: "it was all written down".
//
...
// To the RIGHT of the column, level with its month, stands what each swarm
// reaches: TRAINING (a loop with a packet), EVALUATION (a checklist of fails),
// INFRASTRUCTURE (three racks of five units under the OpenAI mark).
//
...
// edit this file (append-only requests go in WORLD_A_READY.md).
// ---------------------------------------------------------------------------
...
export const STATION_IN: number[] = [90, 94, 98];
...
// ---------------------------------------------------------------------------
// THE STATIONS. One x, level with their months. Icons: outline family, one
// stroke (STROKE), white at INK_LO while untouched.
// ---------------------------------------------------------------------------
export const WAIT_X = COL_X1 + 120; // the waiting zone's centre x (706)
export const ST_CX = COL_X1 + 260 + 55; // 901
...
const ST_CY01 = [monthTop(1) + 107, monthTop(2) + 107];
const EVAL_LABEL_BOTTOM = ST_CY01[1] + ICON_HALF + LABEL_AIR + LABEL_SIZE;
export const MARK_CY = EVAL_LABEL_BOTTOM + LABEL_AIR + MARK_SIZE / 2;
export const RACK_Y0 = MARK_CY + MARK_SIZE / 2 + LABEL_AIR;
const ST_CY = [ST_CY01[0], ST_CY01[1], RACK_Y0 + RACK_BLOCK_H / 2];
...
export const SWARM_SCHED: SwarmSched[] = [
  { station: 0, ignite: 140, birth0: 144, birth1: 160, flow0: 236, flow1: 250, relax0: 272, relax1: 286 },
  { station: 1, ignite: 158, birth0: 162, birth1: 178, flow0: 278, flow1: 292, relax0: 312, relax1: 326 },
  { station: 2, ignite: 176, birth0: 180, birth1: 196, flow0: 326, flow1: 340, relax0: Infinity, relax1: Infinity },
];
...
export const REST_BLOBS = [
  ...
  { x: RACK_X0 - 22 - 78, y: ST_CY[2], rx: 78, ry: 94 },
];
...
export const SWARM_PLANS: DotPlan[][] = SWARM_SCHED.map((sc, s) => {
  ...
  for (let r = 0; r < SWARM_N; r++) {
    ...
    const fT = Math.max(14, Math.min(26, fD / 11));
...
export const swarmDot = (s: number, i: number, G: number): DotState => {
  ...
  if (G < p.fL + p.fT) {
    const u = EASE((G - p.fL) / p.fT);
    if (s < 2) {
      // polar travel round the station: never through the icon
      const q = polarTravel(st, waitAt, envAt, u, 16);
      return { x: q.x, y: q.y, r: baseR, tone: tone0, phase: 3, u };
    }
    const q = curve(waitAt, envAt, u, -0.16);
    return { x: q.x, y: q.y, r: baseR, tone: tone0, phase: 3, u };
  }
  if (G < p.xL) {
    if (p.xL === Infinity) {
...
export const INFRA_CONTACT = (() => {
  for (let G = TABLE_G0; G <= TABLE_G1; G++) {
    for (let i = 0; i < SWARM_N; i++) {
      ...
      if (d.x < RACK_X0 && d.x + d.r >= RACK_X0 - 22 && d.y > RACK_Y0 && d.y < RACK_Y0 + RACK_BLOCK_H) {
...
export const COMPOSITION = { x0: MONTH_LABEL_R - 80, x1: ST_CX + 136, cx: 540 };
...
export const buildCamTable = (
  ...
  for (let f = 0; f <= g1 - g0; f++) {
    ...
    const d = sway(g0 + f);
    rows.push({ cx: x + d.dx, cy: y.cy + d.dy, k: y.k });
  }
...
export const IncidentWorld: React.FC<IncidentWorldProps> = ({
  ...
  for (let li = 0; li <= Math.min(N_LINES - 1, headLine); li++) {
    ...
    for (const wd of L.words) {
      ...
      const lvl = li >= NOW_LINE ? futureAt(EMIT[wd.gi]) * clamp01((li - NOW_LINE + 1) / FUTURE_LINES) : 0;
