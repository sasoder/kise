...
export type FitnessNotIndividualProps = z.infer<typeof schema>;

export const defaultProps: FitnessNotIndividualProps = schema.parse({
  ink: '#FFFFFF',
  accent: '#48D9FF',
...
const FitnessNotIndividual: React.FC<FitnessNotIndividualProps> = ({
  ink,
  accent,
  shadow,
  threadWidth,
            ...
            return (
              <g key={`l${j}`}>
                <line
                  ...
                  strokeLinecap="round"
                  opacity={(0.34 + 0.34 * f) * g * fieldLit}
                />
                <circle
                  cx={a.x + (b.x - a.x) * s}
                  cy={a.y + (b.y - a.y) * s}
          ...
          {agents.map((a, i) => {
            ...
            return (
              <circle
                ...
                r={sizeOf(i)}
                fill={accent}
                opacity={(0.55 + 0.4 * flood(i)) * fieldLit}
              />
            );
          })}
...
export default FitnessNotIndividual;
