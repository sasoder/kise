...
export type FitnessNotIndividualGridEnhancedProps = z.infer<typeof schema>;

export const defaultProps: FitnessNotIndividualGridEnhancedProps = schema.parse({
  ink: '#FFFFFF',
  accent: '#48D9FF',
...
const FitnessNotIndividualGridEnhanced: React.FC<FitnessNotIndividualGridEnhancedProps> = ({
  ink,
  accent,
  shadow,
  threadWidth,
  ...
  // The field drops on a spring, so it settles into place instead of stopping.
  const dropped =
    frame < beats.unlike
      ? 0
      : spring({frame: frame - beats.unlike, fps, config: {damping: 15, stiffness: 68}});
  const fieldY = FIELD_HIGH + (FIELD_LOW - FIELD_HIGH) * dropped;
  ...
  // Each human lands with follow-through.
  const arrive = (h: number) =>
    frame < beats.humans - 8 + h * 5
      ? 0
      : spring({
          frame: frame - (beats.humans - 8 + h * 5),
          fps,
  ...
  // Handed down one descendant at a time: a move, a rest, a move. Not a slide.
  const chargeP = (h: number, k: number) => {
    const raw = clamp01((frame - chargeAt(h, k)) / 26);
    const seg = Math.min(CHAIN.length - 1, Math.floor(raw * CHAIN.length));
    const within = raw * CHAIN.length - seg;
    return (seg + smooth(clamp01(within * 1.6))) / CHAIN.length;
  };
  ...
  const sizeOf = (i: number) =>
    (agents[i].size +
      (EQUAL - agents[i].size) * level +
      RISE * interpolate(flood(i), [0, 0.6, 1], [0, 1.3, 1])) *
      ...
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        <g style={{filter: `drop-shadow(0 2px 9px ${shadow})`}}>
          ...
          {HUMANS.map((h, hi) => {
            ...
            return (
              <g key={`h${hi}`} opacity={Math.min(1, a) * humanFade}>
                {CHAIN.map((y, d) => {
      ...
      {HUMANS.map((h, hi) => (
        <Img
          ...
          style={{
            ...
            opacity: 0.92 * Math.min(1, arrive(hi)) * humanFade,
...
export default FitnessNotIndividualGridEnhanced;
