...
const sgn = (i: number) => (hash(i) > 0.5 ? 1 : -1);
...
const CAM_DAMP = 0.62; // zeta ~0.78, settles in about 13 frames
...
const camera = (upto: number) => {
  ...
  for (let f = 1; f <= upto; f++) {
    ...
    k += vk;
    vcy += (camCy(tk) - cy) * CAM_STIFF - vcy * CAM_DAMP;
    cy += vcy;
  }
  return { cy, k };
};
...
const NicheProductsDoingBetter: React.FC<Props> = ({
  ink,
  accent,
  backgroundBase,
  backgroundSrc,
  ...
  const bgY = -(cy - camCy(CAM_K[0])) * k * parallax - frame * 0.26;
    ...
    <AbsoluteFill style={{ backgroundColor: backgroundBase }}>
      ...
      >
        ...
        >
          ...
          >
            <rect x={X0 - 1300} y={SHELF_Y} width={2600} height={4} fill={ink} opacity={0.26} />
            ...
            {NICHES.map((p) => {
              ...
              const fill = found
                ? interpolateColors(Math.min(age, 4), [0, 4], [ink, accent])
                : ink;
              return (
                <g key={`pr-${p.seed}`}>
...
export default NicheProductsDoingBetter;
