...
export type VastLogoChannelSplitProps = z.infer<typeof schema>;

export const defaultProps: VastLogoChannelSplitProps = schema.parse({
  logo: 'vast.png',
  logoWidth: 860,
  logoAspect: 1968 / 264,
  coreColor: '#FFFFFF',
  // The mark is thin open strokes, so the hard shadow stays well under the
  // ~1.8% used for solid glyphs or it swallows the letterforms.
  shadowOffset: 7,
  rise: 130,
  staggerFrames: 2,
});
...
// The artwork is a flat mark on alpha, so every layer is the logo's own alpha
...
const VastLogoChannelSplit: React.FC<VastLogoChannelSplitProps> = ({
  logo,
  logoWidth,
  logoAspect,
  coreColor,
      ...
      <div style={{position: 'relative', width: logoWidth, height: logoHeight}}>
        ...
        {hasStarted(coreDelay) ? (
          <>
            ...
            {/* Core: the logo's own alpha over flat white, on top of all. */}
            <div
              style={{
                position: 'absolute',
                inset: 0,
...
export default VastLogoChannelSplit;
