...
export type QueenTitrationProps = z.infer<typeof schema>;

export const defaultProps: QueenTitrationProps = schema.parse({
  ink: '#FFFFFF',
  accent: '#48D9FF',
...
const QueenTitration: React.FC<QueenTitrationProps> = ({
  ink,
  accent,
  shadow,
  threadWidth,
  ...
  const drops = pool.map((p, i) => {
    ...
    const slot = Math.max(0, (tDesc - frame) / SPACING);
    ...
    if (frame < tCall) {
      ...
      const t = smooth(clamp01((frame - tOut) / FAN));
