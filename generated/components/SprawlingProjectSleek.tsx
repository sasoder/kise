...
const CAM_K = [1.25, 1.25, 1.2, 1.0, 0.82, 0.8, 1.15, 1.35, 1.35, 1.2, 1.35, 1.35, 1.2, 1.35, 1.3, 1.3, 0.74, 0.74];
const CAM_CX = [540, 540, 540, 540, 540, 540, 440, 380, 380, 470, 540, 540, 640, 720, 720, 720, 540, 540];
...
const SprawlingProjectSleek: React.FC<Props> = ({
  ink,
  accent,
  backgroundBase,
  backgroundSrc,
  ...
  const states: NodeState[] = NODES.map((n, i) => {
    ...
    const pop = spring({ frame: frame - wf, fps, config: { damping: 12, stiffness: 150, mass: 0.7 } });
  ...
  const edgeEl = (
    ...
    to: P,
    drawn: number,
    opts: { width?: number; opacity?: number; phase?: number; depth?: number; reversePulse?: number } = {},
  ) => {
    ...
    const w = (opts.width ?? edgeWidth) + 2 * bump;
...
export default SprawlingProjectSleek;
